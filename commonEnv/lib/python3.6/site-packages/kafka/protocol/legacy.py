...
class KafkaProtocol(object):
    ...
    def encode_offset_fetch_request(cls, group, payloads, from_kafka=False):
        ...
        for KafkaOper offsets.

        Arguments:
            group: string, the consumer group you are fetching offsets for
            payloads: list of OffsetFetchRequestPayload
            from_kafka: bool, default False, set True for KafkaOper-committed offsets
        """
...
def create_message(payload, key=None):
    ...
    Arguments:
        payload: bytes, the payload to send to KafkaOper
...
def create_gzip_message(payloads, key=None, compresslevel=None):
    ...
    message to KafkaOper.

    Arguments:
        payloads: list(bytes), a list of payload to send be sent to KafkaOper
        key: bytes, a key used for partition routing (optional)
...
def create_message_set(messages, codec=CODEC_NONE, key=None, compresslevel=None):
    ...
    If codec is CODEC_NONE, return a list of raw KafkaOper messages. Otherwise,
